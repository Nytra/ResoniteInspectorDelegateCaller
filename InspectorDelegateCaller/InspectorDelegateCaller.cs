...
namespace InspectorDelegateCaller
{
	public class InspectorDelegateCaller : ResoniteMod
	{
		...
		public static HashSet<MethodInfo> GetAllMethodsForRealThisTime(Type t)
		{
		...
		class InspectorDelegateCallerPatch
		{
			static void Postfix(Worker worker, UIBuilder ui)
			{
				...
				worker.World.RunSynchronously(() =>
				{
					...
					var allMethods = GetAllMethodsForRealThisTime(workerType);
					var validMethods = allMethods.Where(methodCheck);
		...
		{
			var syncMethodAttr = info.CustomAttributes.FirstOrDefault((a) => a.AttributeType == typeof(SyncMethod) || a.AttributeType.BaseType == typeof(SyncMethod));
			if (syncMethodAttr != null)
			{
				return true;
			}
			return false;
